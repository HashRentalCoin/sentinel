...
sentinel_cfg = hashrentalcoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_hashrentalcoind_proto_version_with_sentinel_ping = 70207


def get_hashrentalcoin_conf():
    home = os.environ.get('HOME')

    hashrentalcoin_conf = os.path.join(home, ".hashrentalcoincore/hashrentalcoin.conf")
    if sys.platform == 'darwin':
        hashrentalcoin_conf = os.path.join(home, "Library/Application Support/hashrentalcoinCore/hashrentalcoin.conf")

    hashrentalcoin_conf = sentinel_cfg.get('hashrentalcoin_conf', hashrentalcoin_conf)

    return hashrentalcoin_conf
...
hashrentalcoin_conf = get_hashrentalcoin_conf()
network = get_network()
db = get_db_conn()
