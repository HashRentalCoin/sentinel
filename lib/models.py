...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, hashrentalcoind):
        golist = hashrentalcoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_hashrentalcoind(hashrentalcoind, item)
    ...
    @classmethod
    def import_gobject_from_hashrentalcoind(self, hashrentalcoind, rec):
        import decimal
        ...
        # shim/hashrentalcoind conversion
        object_hex = hashrentalcoinlib.SHIM_deserialise_from_hashrentalcoind(object_hex)
        objects = hashrentalcoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from hashrentalcoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from hashrentalcoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from hashrentalcoind - hashrentalcoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(hashrentalcoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from hashrentalcoind! %s" % e)
            govobj.vote_delete(hashrentalcoind)
            return (govobj, None)
    ...
    def vote_delete(self, hashrentalcoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(hashrentalcoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, hashrentalcoind, signal, outcome):
        import hashrentalcoinlib
        ...
        output = hashrentalcoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = hashrentalcoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(hashrentalcoind, signal)

    def sync_network_vote(self, hashrentalcoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = hashrentalcoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 hashrentalcoin addr, non-multisig
            if not hashrentalcoinlib.is_valid_hashrentalcoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid hashrentalcoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (hashrentalcoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/hashrentalcoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import hashrentalcoinlib
        obj_data = hashrentalcoinlib.SHIM_serialise_for_hashrentalcoind(self.serialise())
    ...
    def prepare(self, hashrentalcoind):
        try:
            object_hash = hashrentalcoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not hashrentalcoinlib.is_valid_hashrentalcoin_address(addr, config.network):
    ...
    def hash(self):
        import hashrentalcoinlib
        return hashrentalcoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, hashrentalcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - hashrentalcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, hashrentalcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - hashrentalcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, hashrentalcoind):
        now = int(time.time())
        return (self.created_at < (now - hashrentalcoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, hashrentalcoind):
        if self.is_expired(hashrentalcoind):
            return False

        return True

    def is_deletable(self, hashrentalcoind):
        if self.is_expired(hashrentalcoind):
            return True
