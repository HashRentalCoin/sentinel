...
class GovernanceClass(object):
    ...
    def vote(self, hashrentalcoind, signal, outcome):
        return self.go.vote(hashrentalcoind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, hashrentalcoind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(hashrentalcoind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(hashrentalcoind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to hashrentalcoind "gobject list" output
    ...
    def hashrentalcoind_serialise(self):
        import hashrentalcoinlib
        return hashrentalcoinlib.SHIM_serialise_for_hashrentalcoind(self.serialise())
