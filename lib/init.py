...
def has_hashrentalcoin_conf():
    import config
    import io

    valid_hashrentalcoin_conf = False

    # ensure hashrentalcoin_conf exists & readable
    #
    # if not, print a message stating that hashrentalcoin Core must be installed and
    # configured, including JSONRPC access in hashrentalcoin.conf
    try:
        f = io.open(config.hashrentalcoin_conf)
        valid_hashrentalcoin_conf = True
    except IOError as e:
        print(e)

    return valid_hashrentalcoin_conf
...
def main():
    ...
    if not has_hashrentalcoin_conf():
        print("hashrentalcoinCore must be installed and configured, including JSONRPC access in hashrentalcoin.conf")
        sys.exit(1)
