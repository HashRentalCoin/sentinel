...
def is_valid_hashrentalcoin_address(address, network='mainnet'):
    ...
    hashrentalcoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid hashrentalcoin address
        return False

    if (address_version != hashrentalcoin_version):
        return False
...
# shims 'til we can fix the hashrentalcoind side
def SHIM_serialise_for_hashrentalcoind(sentinel_hex):
    from models import DASHD_GOVOBJ_TYPES
    # unpack
    obj = deserialise(sentinel_hex)

    # shim for hashrentalcoind
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = DASHD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in hashrentalcoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # hashrentalcoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    hashrentalcoind_hex = serialise(obj)
    return hashrentalcoind_hex


# shims 'til we can fix the hashrentalcoind side
def SHIM_deserialise_from_hashrentalcoind(hashrentalcoind_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(hashrentalcoind_hex)

    # shim from hashrentalcoind
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('hashrentalcoin.conf')
